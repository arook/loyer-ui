...
.controller 'ModalCtrl', ($scope, $location, $routeParams, Cart, Store) ->
  $scope.order = {}
  Store.resource('security', 'goods', $routeParams.id).get (rtn) ->
    ...
    Cart.init().$promise.then ->
      angular.forEach Cart.getItem($routeParams.id), (item) ->
  ...
  $scope.ok = ->
    angular.forEach $scope.order, (value, color) ->
        angular.forEach value, (qty, size) ->
            Cart.addItem $routeParams.id, color, size, $scope.item.price, parseInt(qty)
